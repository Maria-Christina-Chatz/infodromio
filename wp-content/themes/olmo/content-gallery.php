...
<?php if(!is_single()): ?>
    <div id="post-<?php the_ID(); ?>" <?php post_class('blog-3-post masonry-image') ?>>
        ...
        <!-- BLOG POST IMAGE -->
        <?php
        $attachments = get_post_meta( get_the_ID(), 'image_gallery', true );
        if( $attachments != '' ){
            $attachments = explode(',', $attachments); ?>
            <div class="blog-media">
                <div class="gallery-carousel owl-carousel" data-items="1" data-nav="true" data-dots="false" data-margin="0" data-autoplay="true">
                    <?php
                    foreach ( $attachments as $attachment ) {
                        $image_url = wp_get_attachment_url($attachment);
                        ?>
                        <div class="item"><img src="<?php echo esc_url($image_url); ?>" alt="<?php echo esc_attr__('images thumbnail', 'olmo'); ?>" /></div>
                        <?php
                    } ?>
                </div>
            </div>
